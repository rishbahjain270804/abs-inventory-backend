  ...
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO items (item_code, item_name, item_category, stock_group, unit_of_measure,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [item_code, item_name, item_category, stock_group, unit_of_measure,
        hsn_code, gst_rate || null, cgst_rate || null, sgst_rate || null, igst_rate || null,
        item_type || 'Stock', opening_quantity || null, opening_value || null,
        minimum_stock_level || null, opening_quantity || 0, active_status || 'Active']
    );
  ...
  try {
    ...
    const [result] = await db.query(
      `UPDATE items SET item_code = ?, item_name = ?, item_category = ?, stock_group = ?, 
       ...
       minimum_stock_level = ?, active_status = ?, updated_at = NOW() WHERE id = ?`,
      [item_code, item_name, item_category, stock_group, unit_of_measure,
        hsn_code, gst_rate || null, cgst_rate || null, sgst_rate || null, igst_rate || null, item_type,
        opening_quantity || null, opening_value || null, minimum_stock_level || null,
        active_status, req.params.id]
    );
